...
public:
	...
	FVector(float _X, float _Y) : X(_X), Y(_Y), Z(0.f), W(1.f)
	{

	}

	FVector(float _X, float _Y, float _Z) : X(_X), Y(_Y), Z(_Z), W(1.f)
	{
	...
	FVector(int _X, int _Y) : X(static_cast<float>(_X)), Y(static_cast<float>(_Y)), Z(0.f), W(1.f)
	{

	}

	FVector(long _X, long _Y) : X(static_cast<float>(_X)), Y(static_cast<float>(_Y)), Z(0.f), W(1.f)
	{
...
public:
	...
	void OrthographicLH(float _Width, float _Height, float _Near, float _Far)
	{
		...
		float fRange = 1.f / (_Far - _Near);

		Arr2D[0][0] = 2.f / _Width;
		Arr2D[1][1] = 2.f / _Height;
		Arr2D[2][2] = fRange;
	...
	void PerspectiveFovRad(float _FovAngle, float _Width, float _Height, float _Near, float _Far)
	{
		...
		float DivFov = _FovAngle / 2.f;

		Arr2D[2][3] = 1.f;
		Arr2D[3][3] = 0.f;
		Arr2D[0][0] = 1.f / (tanf(DivFov) * ScreenRatio);
		Arr2D[1][1] = 1.f / tanf(DivFov);
		Arr2D[2][2] = (_Far + _Near) / (_Far - _Near);
		Arr2D[3][2] = -2.f * (_Near * _Far) / (_Far - _Near);
	}
	...
	void ViewPort(float _Width, float _Height, float _Left, float _Top, float _ZMin, float _ZMax)
	{
		...
		Arr2D[3][2] = _ZMax != 0.f ? 1.f : _ZMin / _ZMax;
	}
